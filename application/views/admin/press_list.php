        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    ...
                    <?}?>
                    <button type="button" id="idxgo" class="btn btn-primary" data-url="/admin/press-order" data-count="/admin/press-count" data-page="<?=$page?>">순서 저장하기</button></a>
                    <div class="clearfix"></div>
                </div>
                <div class="x_content">
                                    ...
                                    <tr class="even pointer" data-idx="<?=$data->idx?>" style="display:none;">
                                        <?$i=0; foreach ($data as $subkey => $value):?>
                                            ...
                                            <?if(strpos($subkey,'idx') === false):?>
                                                <?if($subkey == 'thumbnail'):?>
                                                    <td class=""><img width="150" src="<?=$value ? '/assets/uploads/'.$value : '/assets/images/xbox.jpg'?>"></td>
                                                <?else:?>
                                                    <td class=""><?=$value?></td>
                                                <?endif;?>
                                            <?endif;?>
...
<script>
    $('body').on('click', '.adminedit', function() {
        ...
        location.href = '/admin/press-edit/'+idx;
    });
    $('body').on('click', '.admindel', function() {
        var con = confirm("정말 삭제하시겠습니까?");
        if(con) {
            var idx = $(this).parent().parent().attr('data-idx');
            var result = jsonreturn('/admin/press-del/'+idx);
